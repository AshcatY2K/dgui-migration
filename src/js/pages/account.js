...
account.get()
  .then( async ( account ) => {
    ...
    if ( "Country" in account ) {
      $( "#country" ).val( account["Country"] );
    }

    if ( "payPalSubscriptionId" in account ) {
...
$( '[data-mdb-target="#accountItem"]' ).on( "click", e => {
  $( "#account .form-outline" ).each( ( i, el ) => {
    let width = $( el ).find( "label" ).width();

    $( el ).find( ".form-notch-middle" ).width( width );
  } );
} );
